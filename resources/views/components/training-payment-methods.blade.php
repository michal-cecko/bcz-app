...
<div class="w-full flex flex-col gap-3">
    ...
    @if(in_array('bank_transfer', $enabledMethods))
        <button wire:click="selectPaymentMethod('bank_transfer')" class="w-full flex items-center gap-4 px-6 py-5 rounded-xl bg-[#0A0A0A] transition {{ $selectedPaymentMethod === 'bank_transfer' ? 'border-2 border-[#FF2D2D]' : 'border border-[#333333] hover:border-[#555555]' }}">
            ...
            <div class="flex flex-col items-start gap-0.5 flex-1">
                <span class="text-white text-sm font-semibold">{{ __('training_detail.payment_bank_transfer') }}</span>
                <span class="text-[#666666] text-xs">{{ __('training_detail.payment_bank_transfer_desc') }}</span>
            </div>
    ...
    @if(in_array('cash', $enabledMethods))
        <button wire:click="selectPaymentMethod('cash')" class="w-full flex items-center gap-4 px-6 py-5 rounded-xl bg-[#0A0A0A] transition {{ $selectedPaymentMethod === 'cash' ? 'border-2 border-[#FF2D2D]' : 'border border-[#333333] hover:border-[#555555]' }}">
            ...
            <div class="flex flex-col items-start gap-0.5 flex-1">
                <span class="text-white text-sm font-semibold">{{ __('training_detail.payment_cash') }}</span>
                <span class="text-[#666666] text-xs">{{ __('training_detail.payment_cash_desc') }}</span>
            </div>
...
@if($selectedPaymentMethod === 'cash')
    <div class="rounded-xl bg-[#0A0A0A] border border-[#FF2D2D]/20 p-5 flex flex-col gap-4 w-full">
        ...
        <div class="flex flex-col gap-2.5">
            @foreach([
                __('training_detail.cash_step_1'),
                __('training_detail.cash_step_2'),
                __('training_detail.cash_step_3'),
            ] as $index => $stepText)
        ...
        <div class="flex items-center gap-2 rounded-lg bg-[#F59E0B]/[0.06] border border-[#F59E0B]/[0.18] px-3 py-2.5 w-full">
            ...
            <span class="text-[#F59E0B] text-[10px] font-medium">{{ __('training_detail.cash_warning') }}</span>
        </div>
    </div>
@endif
