...
@if($hasStats)
    ...
    <div class="flex flex-col gap-4 p-5" style="width: 280px; background-color: {{ $bgColor }}; box-shadow: 0 16px 48px rgba(0,0,0,0.25)">
        {{-- Header: icon --}}
        <div class="w-12 h-12 flex items-center justify-center" style="background-color: #FF2D2D15">
        ...
        <div class="flex gap-3 w-full">
            @if(!empty($stat1_value))
                <div class="flex-1 flex flex-col items-center gap-0.5 {{ $isLight ? 'bg-[#F5F5F5]' : 'bg-white/[0.05]' }} p-3">
        ...
        {{-- CTA Button --}}
        @if($btnText && $btnUrl)
            <a href="{{ $btnUrl }}" class="flex items-center justify-center gap-2 w-full py-3 bg-[#FF2D2D] text-white text-[11px] font-bold font-['DM_Sans'] tracking-widest hover:bg-[#E02626] transition-colors">
                {{ $btnText }}
...
@else
    ...
    <div class="flex items-center gap-4 border border-[#222222] p-4 pr-5 w-[480px] max-w-[calc(100vw-2rem)]" style="background-color: {{ $bgColor }}; box-shadow: 0 8px 32px rgba(0,0,0,0.38)">
        {{-- Icon --}}
        <div class="shrink-0 w-10 h-10 bg-[#FF2D2D]/[0.08] flex items-center justify-center">
        ...
        {{-- Button --}}
        @if($btnText && $btnUrl)
            <a href="{{ $btnUrl }}" class="shrink-0 inline-flex items-center justify-center h-[34px] px-4 bg-[#FF2D2D] text-white text-[11px] font-bold font-['DM_Sans'] hover:bg-[#E02626] transition-colors">
                {{ $btnText }}
            </a>
        @endif
    </div>
