    ...
    <div class="bg-[#0D0D0D] rounded-xl p-8 md:p-10 mb-8">
        <div class="flex flex-col gap-6">
            ...
            <div class="flex flex-wrap items-center gap-4">
                <span class="text-[#888888] text-sm font-semibold">{{ __('coaches_archive.filter') }}:</span>
                <select wire:model.live="categoryFilter" class="bg-[#111111] border border-[#333333] text-[#CCCCCC] text-sm rounded-lg px-4 py-2.5 focus:border-bcz-red focus:ring-0 outline-none appearance-none cursor-pointer">
                    <option value="">{{ __('coaches_archive.all_specializations') }}</option>
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}">{{ $category->getTranslation('name', app()->getLocale()) }}</option>
                    @endforeach
                </select>
                @if($this->hasActiveFilters())
                    <button wire:click="resetFilters" class="text-[#888888] text-sm hover:text-white transition-colors">{{ __('coaches_archive.reset') }}</button>
                @endif
            </div>
