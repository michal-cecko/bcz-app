<div>
    ...
    <div class="bg-[#0D0D0D] rounded-xl p-8 md:p-10 mb-8">
        <div class="flex flex-col gap-6">
            ...
            <div class="relative">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('judges_archive.search_placeholder') }}" class="bg-[#111111] border border-[#222222] text-white text-[15px] rounded-lg pl-12 pr-5 py-4 focus:border-bcz-red focus:ring-0 outline-none w-full placeholder-[#666666]">
            </div>

            {{-- Filters --}}
            <div class="flex flex-wrap items-center gap-4">
                <span class="text-[#888888] text-sm font-semibold">{{ __('judges_archive.filter') }}:</span>
                <select wire:model.live="categoryFilter" class="bg-[#111111] border border-[#333333] text-[#CCCCCC] text-sm rounded-lg px-4 py-2.5 focus:border-bcz-red focus:ring-0 outline-none appearance-none cursor-pointer">
                    <option value="">{{ __('judges_archive.all_specializations') }}</option>
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}">{{ $category->getTranslation('name', app()->getLocale()) }}</option>
                    @endforeach
                </select>
                @if($this->hasActiveFilters())
                    <button wire:click="resetFilters" class="text-[#888888] text-sm hover:text-white transition-colors">{{ __('judges_archive.reset') }}</button>
                @endif
            </div>
    ...
    <div class="flex items-center justify-between mb-8">
        <span class="text-[#888888] text-sm">{{ __('judges_archive.showing', ['count' => $judges->total()]) }}</span>
    </div>
                ...
                <div class="bg-[#111111] border border-[#222222] rounded-2xl overflow-hidden flex flex-col">
                    ...
                    <div class="p-6 flex flex-col gap-4 flex-1">
                        <div class="flex items-start justify-between gap-3">
                            <h3 class="text-white text-xl font-bold">{{ $judge->name }}</h3>
                        </div>

                        @if($judge->certifications->isNotEmpty())
                            <div class="flex flex-wrap gap-2">
                                @foreach($judge->certifications->take(3) as $cert)
                        ...
                        <a href="{{ route('judge.show', $judge) }}" class="flex items-center justify-center gap-2 bg-[#0A0A0A] border border-[#333333] text-white text-sm font-semibold rounded-lg py-3.5 mt-auto hover:border-[#555555] transition-colors">
                            {{ __('judges_archive.view_profile') }}
                            <span>→</span>
                        </a>
                    </div>
                </div>
    ...
    @else
        <div class="text-center py-20">
            <p class="text-[#888888] text-lg">{{ __('judges_archive.no_results') }}</p>
        </div>
    @endif
</div>
