<section class="py-[60px]">
 ...
 <div class="bg-[#111111] p-8 border border-[#222222] flex flex-col gap-6">
 <div class="flex items-center gap-4">
 <div class="w-12 h-12 flex items-center justify-center text-white"style="background-color: {{ $color }}">
 @if(! empty($card['icon']))
 ...
 <div class="w-6 h-6 flex items-center justify-center text-white text-xs font-bold shrink-0"style="background-color: {{ $color }}">{{ $stepIndex + 1 }}</div>
 ...
 <a href="{{ $buttonHref }}"class="text-white py-3.5 w-full text-center font-semibold block"style="background-color: {{ $color }}">{{ brick_trans($card['button_text'] ?? []) }}</a>
 @endif
 </div>
 @endforeach
 </div>
