...
<section class="bg-[#111111] pb-[100px]">
 ...
 @endphp
 <{{ $cardTag }} @if($cardUrl) href="{{ $cardUrl }}"@endif class="bg-bcz-dark border overflow-hidden flex flex-col group transition-all duration-300 {{ $cardUrl ? 'hover:border-opacity-80 hover:-translate-y-1 hover:shadow-lg hover:shadow-black/20' : '' }}"style="border-color: {{ $card['border_color'] ?? '#222222' }}">
 @if(! empty($card['image']))
 ...
 @if(! empty($card['icon']))
 <div class="size-12 flex items-center justify-center self-start"style="background-color: {{ $accentColor }}20">
