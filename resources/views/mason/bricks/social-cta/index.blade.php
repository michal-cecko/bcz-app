<section class="relative w-full h-[350px] md:h-[420px] lg:h-[500px] overflow-hidden">
    @php $bgUrl = brick_media_url($background_image ?? null); @endphp
    @if($bgUrl)
    ...
    <div class="relative w-full h-full flex flex-col items-center justify-center">
        ...
        @if(! empty($title))
            <h2 class="font-display font-bold text-[28px] md:text-[40px] lg:text-[56px] tracking-wide text-center">{{ brick_trans($title) }}</h2>
        @endif

        @if(! empty($description))
            <p class="text-bcz-muted text-lg text-center">{!! brick_trans($description) !!}</p>
        @endif

        @if(! empty($resolved_instagram) || ! empty($resolved_facebook) || ! empty($resolved_youtube))
            <div class="flex items-center gap-6">
                @if(! empty($resolved_instagram))
                    <a href="{{ $resolved_instagram }}" class="w-10 h-10 md:w-[60px] md:h-[60px] bg-bcz-red flex items-center justify-center hover:bg-red-700 transition-colors">
                ...
                @endif
                @if(! empty($resolved_facebook))
                    <a href="{{ $resolved_facebook }}" class="w-10 h-10 md:w-[60px] md:h-[60px] border-2 border-white flex items-center justify-center hover:bg-white/10 transition-colors">
                ...
                @endif
                @if(! empty($resolved_youtube))
                    <a href="{{ $resolved_youtube }}" class="w-10 h-10 md:w-[60px] md:h-[60px] border-2 border-white flex items-center justify-center hover:bg-white/10 transition-colors">
