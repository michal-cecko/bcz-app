<section class="bg-[#0D0D0D] py-[60px]">
    <div class="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-20 flex flex-col items-center gap-8">
        ...
        {{-- Card --}}
        @if(! empty($rows))
            <div class="rounded-2xl bg-[#111111] border border-[#222222] p-8 max-w-[700px] w-full flex flex-col">
                @foreach($rows as $i => $row)
                    <div class="flex items-center justify-between py-4 {{ $i < count($rows) - 1 ? 'border-b border-bcz-border' : '' }}">
                        ...
                        >
                            ...
                            <span
                                class="absolute -top-8 right-0 text-white text-xs px-2 py-1 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"
