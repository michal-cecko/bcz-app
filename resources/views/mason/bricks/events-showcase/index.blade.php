...
<section class="py-[100px]">
    <div class="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-20">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 mb-12">
            ...
            @endphp
            @if($viewAllText)
                <a href="{{ $viewAllUrl }}" class="inline-flex items-center gap-2 border border-bcz-red text-bcz-red rounded-lg px-6 py-3 text-sm font-semibold hover:bg-bcz-red hover:text-white transition shrink-0">
                    {{ $viewAllText }}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($events as $event)
                ...
                @endphp
                <a href="/eventy/{{ $event->slug }}" class="bg-[#111111] rounded-2xl overflow-hidden group hover:ring-1 hover:ring-[#333333] transition">
                    {{-- Image --}}
                    @if($eventImage)
                    ...
                    <div class="p-5 flex flex-col gap-3">
                        @if($categoryTitle)
                            <div>
                                <span class="inline-flex items-center rounded px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase"
