...
@section('content')
 ...
 {{-- Badge --}}
 <div class="flex items-center gap-2 bg-transparent border border-bcz-red/30 px-4 py-2 w-fit">
 ...
 <a href="{{ route('team.competition.show', [$competition->team, $competition]) }}"class="bg-[#111111] border border-[#222222] overflow-hidden flex flex-col md:flex-row hover:border-[#333333] transition-colors group">
 {{-- Date Column --}}
 ...
 @endif
 @if(!$team && $competition->team)
 <x-team-badge :team="$competition->team"/>
 @endif
 </div>
 </div>
 {{-- CTA Column --}}
 <div class="flex items-center px-6 pb-6 md:pb-0 md:pr-8">
 <span class="{{ $loop->first ? 'bg-bcz-red text-white hover:bg-red-700' : 'border border-[#444444] text-white hover:border-bcz-red' }} text-[12px] font-bold tracking-wider px-6 py-3 transition-colors whitespace-nowrap">
 DETAIL
 </span>
 </div>
 </a>
 ...
 <a href="{{ route('team.competition.show', [$competition->team, $competition]) }}"class="bg-[#1A1A1A] border border-[#222222] overflow-hidden group hover:border-[#333333] transition-colors">
 ...
 @endif
 <div class="absolute top-4 left-4 bg-[#222222] text-[#AAAAAA] text-[11px] font-bold tracking-wider px-3 py-1.5">UKONČENÁ</div>
 </div>
 ...
 @endif
 <h3 class="font-display font-bold text-[24px] tracking-wide text-white">{{ $competition->getTranslation('title', app()->getLocale()) }}</h3>
 @if($competition->city || $competition->country)
