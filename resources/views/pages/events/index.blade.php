...
@section('title', 'Podujatia - BCZ Club')
...
@section('content')
    ...
    <section class="bg-bcz-dark pt-[120px] pb-[60px]">
        <div class="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-20 flex flex-col items-center gap-6">
            ...
            {{-- Title --}}
            <h1 class="font-display font-bold text-[28px] md:text-[40px] lg:text-[56px] tracking-wide text-center text-white">Podujatia</h1>

            {{-- Description --}}
            <p class="text-[#888888] text-[18px] text-center leading-relaxed max-w-[700px]">
                Prehľad všetkých našich vystúpení, prednášok a workshopov.
            </p>
        ...
        <div class="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-20">
            @if($events->isEmpty())
                <div class="text-center py-20">
                    <p class="text-[#666666] text-lg">Momentálne nie sú k dispozícii žiadne podujatia.</p>
                </div>
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($events as $event)
                        <a href="{{ route('event.show', $event) }}" class="rounded-2xl bg-[#111111] overflow-hidden flex flex-col group hover:ring-1 hover:ring-[#333333] transition-all">
                            <div class="h-[180px] bg-[#1A1A1A] overflow-hidden">
                                @if($event->card_image)
                                    <img src="{{ $event->card_image }}" alt="{{ $event->getTranslation('title', app()->getLocale()) }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @endif
                            </div>
                            <div class="p-5 flex flex-col gap-3">
                                <div class="flex items-center justify-between">
                                    @if($event->eventCategory)
                                        <span class="text-xs px-2.5 py-1 rounded"
